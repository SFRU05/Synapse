...
async def help_slash(
    ...
    if cat == "일반":
        ...
        embed.add_field(name="유저정보", value="유저 정보를 확인합니다.", inline=False)
        embed.add_field(name="서버정보", value="서버의 정보를 확인합니다.", inline=False)
        embed.add_field(name="봇정보", value="봇의 정보를 확인합니다.", inline=False)
        embed.add_field(name="아바타", value="사용자의 아바타를 불러옵니다.", inline=False)
        await interaction.response.send_message(embed=embed, ephemeral=True)
    elif cat == "관리":
        embed = discord.Embed(title="관리 명령어", description="관리 명령어 목록이에요.", color=discord.Color.red())
        embed.add_field(name="킥 <유저 멘션 / ID 입력> <사유>", value="해당 유저를 추방합니다.", inline=False)
        embed.add_field(name="밴 <유저 멘션 / ID 입력> <사유>", value="해당 유저를 밴합니다.", inline=False)
        embed.add_field(name="타임아웃 <유저 멘션 / ID 입력> <시간(분)> <사유>", value="사용자를 타임아웃합니다.", inline=False)
        ...
        embed.add_field(name="주식 <종목명> <간격>", value="해당 주식의 현재 주가를 확인합니다.", inline=False)
        embed.add_field(name="관심종목", value="등록한 관심 종목을 확인합니다.", inline=False)
        await interaction.response.send_message(embed=embed, ephemeral=True)
    else:
