...
@app_commands.command(name="타임아웃", description="유저를 일정 시간(분) 타임아웃해요.")
...
async def timeout_slash(
    ...
    if not interaction.user.guild_permissions.moderate_members:
        ...
        return
    if duration <= 0:
        await interaction.response.send_message("타임아웃(분) 시간은 1 이상으로 설정해주세요.", ephemeral=True)
        return
    ...
    embed = discord.Embed(
        title="타임아웃 확인",
        description=f"{member.mention} 님을 {int(hours)}시간 {int(minutes)}분 동안 타임아웃할까요?",
    ...
    if view.result == "confirm":
        try:
            ...
            public_embed = discord.Embed(
                title="타임아웃 완료",
                description=f"{member.mention} 님이 {int(hours)}시간 {int(minutes)}분 동안 타임아웃되었어요.",
            ...
            await interaction.edit_original_response(content="❌ 봇에 타임아웃 권한이 없어요.", embed=None, view=None)
        except discord.HTTPException:
            await interaction.edit_original_response(content="❌ 타임아웃 요청 중 오류가 발생했어요.", embed=None, view=None)
    elif view.result == "cancel":
        cancel_embed = discord.Embed(
            title="타임아웃 요청 취소",
            description=f"{member.mention} 님에 대한 타임아웃 요청이 취소되었어요.",
        ...
        timeout_embed = discord.Embed(
            title="시간 초과",
            description="15초 내 선택이 없어 타임아웃 요청이 취소되었어요.",
            color=discord.Color.light_grey()
        )
...
async def pardon_slash(
    ...
    embed = discord.Embed(
        title="타임아웃 해제 확인",
        description=f"{member.mention} 님의 타임아웃을 해제할까요?",
    ...
    if view.result == "confirm":
        try:
            ...
            public_embed = discord.Embed(
                title="타임아웃 해제 완료",
                description=f"{member.mention} 님의 타임아웃이 해제되었어요.",
                color=discord.Color.green()
            )
            ...
            await interaction.edit_original_response(content="❌ 봇에 타임아웃 해제 권한이 없어요.", embed=None, view=None)
        except discord.HTTPException:
            await interaction.edit_original_response(content="❌ 타임아웃 해제 요청 중 오류가 발생했어요.", embed=None, view=None)
    elif view.result == "cancel":
        cancel_embed = discord.Embed(
            title="타임아웃 해제 요청 취소",
            description=f"{member.mention} 님에 대한 타임아웃 해제 요청이 취소되었어요.",
            color=discord.Color.light_grey()
        )
        await interaction.edit_original_response(embed=cancel_embed, view=None)
    else:
        timeout_embed = discord.Embed(
            title="시간 초과",
            description="15초 내 선택이 없어 타임아웃 해제 요청이 취소되었어요.",
            color=discord.Color.light_grey()
        )
        await interaction.edit_original_response(embed=timeout_embed, view=None)